...
Estimating the parameters $\mu$ and $\sigma^2$ from a training dataset for the purpose of anomaly detection can be done by calculating the average of the training examples to get $\mu$ and the mean squared difference from the mean of the training examples to get $\sigma^2$. These calculation are called the maximum likelihood estimates of $\mu$ and $\sigma$.
